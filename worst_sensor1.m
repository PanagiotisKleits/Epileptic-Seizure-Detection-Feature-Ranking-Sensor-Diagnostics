...
H_FEATURES_PRE(:,:,i_event)= [Sensor_Activity Sensor_Mobility Sensor_Complexity];
end


H_FEATURES_ICTAL=[];
for i_event=1:8, X=ICTAL_DATA(:,:,i_event); 
Sensor_Activity=var(X,[],2);
DIFF1=diff(X,1,2);
DIFF2=diff(X,2,2);
Sensor_Mobility=std(DIFF1,[],2)./std(X,[],2); 
Sensor_Complexity= (std(DIFF2,[],2)./std(DIFF1,[],2))./Sensor_Mobility;
H_FEATURES_ICTAL(:,:,i_event)= [Sensor_Activity Sensor_Mobility Sensor_Complexity]; 
end



AFVs=[];
for ii=1:76, AFVs=[AFVs,squeeze(H_FEATURES_ICTAL(ii,:,:))'];
end 
% ICTAL state
BFVs=[];
for ii=1:76, BFVs=[BFVs,squeeze(H_FEATURES_PRE(ii,:,:))'];
end  
% PRE-ICTAL state
...
worst_sensor=ceil(RANK(end)/3);
worst_attribute=[(rem(RANK(end),3)==0)*3 ]+ [rem(RANK(end),3)~=0]*[rem(RANK(end),3)];

display(strcat('worst sensor: ',num2str(worst_sensor)))
display(strcat('worst attribute: ',num2str(worst_attribute)))



figure,

idx_worst = (3*(worst_sensor-1)+1):(3*worst_sensor);
X_worst = XX(idx_worst, :)';    % 16x3 πίνακας (δείγματα x χαρακτηριστικά)
Y = GROUP(:);
N = length(Y);
C_pred = zeros(N,1);
...
for i = 1:N
    ...
    testIdx = i;
    C_pred(i) = classify(X_worst(testIdx,:), X_worst(trainIdx,:), Y(trainIdx));
end

cp_worst = classperf(Y, C_pred);
fprintf('Classification performance using ONLY the worst sensor (29): %.2f%%\n', cp_worst.CorrectRate*100);

for ii=1:8
    subplot(2,4,ii),plot([1:4000],ICTAL_DATA(worst_sensor,:,ii),'r',[1:4000],PRE_DATA(worst_sensor,:,ii),'b'),
